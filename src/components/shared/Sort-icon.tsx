...
export function SortIcon<TKeys>(props: SortIconProps<TKeys>) {
  ...
  if (sort !== currentSort) {
    return <ArrowUpDown className=" w-4 h-4"></ArrowUpDown>;
  }

  return currentOrder === "asc" ? (
    <ArrowUpNarrowWide className=" w-4 h-4"></ArrowUpNarrowWide>
  ) : (
    <ArrowUpNarrowWide className=" w-4 h-4 rotate-180"></ArrowUpNarrowWide>
  );
}
